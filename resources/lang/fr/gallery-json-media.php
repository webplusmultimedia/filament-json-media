<?php

declare(strict_types=1);

// translations for GalleryJsonMedia/FilamentJsonMedia
return [
    'Drag&Drop' => 'Drag & Drop ou cliquer ICI',
    'tooltip.edit-button-custom-property' => 'Editer les propriétés personnalisées',
    'tooltip.delete-media-button' => 'Supprimer le média',
    'title.edit-modal-form-for-customs-properties' => 'Edition des propriétés personnalisées',
    'infoList.document.nothing-to-show' => 'Pas de document',
    'form.alt.label.media' => 'Alt',
    'form.alt.label.document' => 'Titre du Document',
    'errors' => [
        'model_interface_missing' => 'Interface HasMedia non implémentée sur le Model (:model)',
    ],
];
